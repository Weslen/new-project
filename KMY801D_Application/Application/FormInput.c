...
void _FormInput123ReadCode (unsigned char *pRead, unsigned char keyval)
{
	switch (keyval)
	{
		case KEY0:
...
unsigned char FormInput (struct sFormInput *pFI, unsigned char TitleNumber)
{
	...
	while (1)
	{
		...
		switch (keyval)
		{
			...
			case KEY9:
			case KEYXING:
				if (strlen ( (const char*) (pt->TitleResult) ) >= pt->MaxInputSize)
				{
					KeyIsResponse();
					continue;
				}
				...
				if (inputMode == INPUT_MODE_123)
				{
					...
					if (temp >= pt->MaxInputSize)
					{
						KeyIsResponse();
						continue;
					}

					_FormInput123ReadCode (buff, keyval);
					buff[1] = 0;
				}
