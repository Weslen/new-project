	...
	while (1)
	{
		...
		switch (keyval)
		{
			...
			case KEY_DOWN:
				...
				//printf("MenuHandler1 MenuOp.Menu=%s,addr=%d\r\n",(*MenuOp).Menu->MenuStrOther,(*MenuOp).Menu->MenuFuc);
				if ( (*MenuOp).Menucnt < (*MenuOp).MenuTotal - 1) //未到最后一个菜单
				{
					(*MenuOp).Menu++;
					(*MenuOp).Menucnt++;
				...
				//printf("MenuHandler3 MenuOp.Menu=%s,addr=%d\r\n",(*MenuOp).Menu->MenuStrOther,(*MenuOp).Menu->MenuFuc);
				break;
	...
	while (1)
	{
		...
		else if ( ( (*MenuOp).flashflag & FlushAllFlag) != 0)
		{
			...
			for (i = (*MenuOp).StartLine; i < (*MenuOp).EndLine && i < (*MenuOp).MenuTotal + (*MenuOp).StartLine; i++)
			{
				if ( (*MenuOp).Ico != NULL)
				{
					steaddraw_pic_to_row_x ( (*MenuOp).Ico, 0, i, 12, 12);
				}
