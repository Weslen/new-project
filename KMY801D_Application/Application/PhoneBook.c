...
static const unsigned char PhoneNameIco[24]={0xf0,0xfe,0xfe,0xff,0x9f,0x0f,0x07,0x07,0x93,0xdf,0x26,0x00,0x00,0x09,0x0f,0x0b,0x08,0x00,0x0a,0x06,0x0a,0x01,0x00,0x00};
static const unsigned char PhoneNumberIco[24]={0x00,0xfc,0x24,0x64,0x24,0x64,0x24,0x65,0x27,0xfd,0x00,0x00,0x00,0x0f,0x08,0x0d,0x08,0x0d,0x08,0x0d,0x08,0x0f,0x00,0x00};
static const unsigned char PhoneZuoJiIco[24]={0x06,0xe7,0xf1,0xfd,0xb9,0x59,0xb9,0xfd,0xf1,0xe7,0x06,0x00,0x00,0x07,0x07,0x07,0x07,0x07,0x07,0x07,0x07,0x07,0x00,0x00};
static const unsigned char PhoneEmailIco[24]={0xfc,0xfe,0x03,0xf9,0xfd,0x05,0x05,0xfd,0xfd,0x02,0xfe,0xfc,0x01,0x03,0x06,0x04,0x05,0x05,0x05,0x05,0x05,0x05,0x01,0x00};
static const unsigned char PhoneBeiZhuIco[24]={0x00,0x87,0x67,0x1f,0x18,0x28,0xc8,0x08,0x08,0x88,0x68,0x18,0x06,0x05,0x04,0x04,0x04,0x04,0x04,0x04,0x06,0x01,0x00,0x00};
static const unsigned char PhoneSaveIco[24]={0xff,0xff,0xc3,0x55,0x55,0x55,0x55,0x55,0xc3,0xff,0xff,0x00,0x07,0x07,0x04,0x04,0x05,0x04,0x04,0x04,0x04,0x07,0x07,0x00};


static const unsigned char con_Chinese_PhoneNumber[]={"手机号码"},con_Chinese_PhoneName[]={"姓名"},con_Chinese_PhoneZuoJi[]={"座机号码"},con_Chinese_PhoneEmail[]={"邮箱"},con_Chinese_PhoneBeiZhu[]={"备注"},con_Chinese_PhoneSave[]={"保存"};
static const unsigned char con_English_PhoneNumber[]={"Mobile number"},con_English_PhoneName[]={"Name"},con_English_PhoneZuoJi[]={"Telephone"},con_English_PhoneEmail[]={"E-mail"},con_English_PhoneBeiZhu[]={"Remark"},con_English_PhoneSave[]={"Save"};
...
static const unsigned char file_phonebook[]={"PhoneBook"};
...
static unsigned char PhoneAddName(unsigned char *receive,u16 size)
{
...
//删除一个记录
static char PhoneDeleteARecord(unsigned char Record)
{
...
//按拼音排序保存一个记录
static unsigned char PhoneAddSave(unsigned char *receive,u16 size)
{
...
{
	static unsigned char PhoneNumber[18],PhoneName[12],PhoneZuoJi[18],PhoneEmail[20],PhoneBeiZhu[30],PhoneSave[5];
	char temp[50];
	int numPhone;
	EFILE efile;
	...
	EepromFile_Open((unsigned char*)file_phonebook,&efile);
		...
		if(MenuOp.RetVal==KEY_Enter){
			if(MenuOp.Menucnt==MenuOp.MenuTotal-1){
				if(PhoneDeleteARecord(numPhone)==RIGHT_return){
					MenuOp.FucRetVal=MenuOp.Menu->MenuFuc((unsigned char *)Menu,0);
				}
...
{
	unsigned char phonebuff[20];
	unsigned char atcmd[50];
	EFILE efile;
	...
	phonebuff[0]=EepromFile_Open((unsigned char*)file_phonebook,&efile);
...
{
	static unsigned char PhoneNumber[18],PhoneName[12],PhoneZuoJi[18],PhoneEmail[20],PhoneBeiZhu[30];
	...
	retval=EepromFile_Open((unsigned char*)file_phonebook,&efile);
...
static char PhoneExplorer(char *msg)
{
	...
	keyval=EepromFile_Open((unsigned char*)file_phonebook,&efile);
...
static char PhoneDeleteAll(char *msg)
{
	...
	if(retval==KEY_Enter){
		//system("rm PhoneBook");
		EepromFile_Delete((unsigned char*)file_phonebook);
		clear_lcd();
