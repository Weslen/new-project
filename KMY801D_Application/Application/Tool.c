...
static char Calculator (char *msg) //2.计算器
{
	...
	while (1)
	{
		...
		switch (keyval)
		{
			...
			case KEY_DEL:	//删除返回键
				if (inputcnt)
				{
					...
					if (ope == WaitFirst_inputed)
					{
						abuff[inputcnt] = 0;
						text_out_to_row_fillSpace (1, abuff);
					}
					else if (ope == WaitSecond_inputed)
					{
						bbuff[inputcnt] = 0;
						text_out_to_row_fillSpace (3, bbuff);
					}
				}
				else
				{
		...
		if (inputFlag == 1 && inputcnt < sizeof (bbuff) - 3)  			//如果有0到9.输入并且没有超过规定的位数
		{
			...
			{
				if (ope == WaitFirst || ope == WaitFirst_inputed)
				{
					ope = WaitFirst_inputed;
					abuff[inputcnt++] = '0';
					abuff[inputcnt++] = '.';
				...
				else if (ope == WaitSecond || ope == WaitSecond_inputed)
				{
					ope = WaitSecond_inputed;
					bbuff[inputcnt++] = '0';
					bbuff[inputcnt++] = '.';
					bbuff[inputcnt] = 0;
					text_out_to_row_fillSpace (3, bbuff);
