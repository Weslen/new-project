...
{
	const static struct MenuFuc_Language2 Menu[8] =
	{
...
{
	const static struct MenuFuc_Language2 Menu[7] =
	{
...
static signed char CheckPINAndPUK (void)
{
	...
	if (strstr ( (const char*) CmdBuff, "+CME ERROR: 3517") )
	{
		clear_lcd();
	...
	if (strstr ( (const char*) CmdBuff, "SIM PIN")!=NULL )
	{
		...
		if(strlen((char*)SIM_PIN)==0)
		{
			SIM_PINset(NULL);
		}
		strcpy ( (char *) CmdBuff, "AT+CPIN=\"");
		strcat ( (char *) CmdBuff, (const char*) SIM_PIN);
		...
		if (strstr ( (const char*) CmdBuff, "SIM PIN")!=NULL )
		{
			PassWordInput2 ("Input PIN", SIM_PIN, 6, 1, 0, KEY_NONE, 1);
			StoreLocalSettings();
		}
		memset(CmdBuff,0,sizeof(CmdBuff));
		strcpy ( (char *) CmdBuff, "AT+CPIN=\"");
...
unsigned char Sim900B_init (void)
{
	...
	send_at_cmd("AT+CMGF=1\r"); // Set SMS text mode
	kmy_USART2ReceiveData(buff,sizeof(buff),10);
	printf("%s\r\n",buff);
	if(strstr((char*)buff,"+CME ERROR: 3517")!=0)
	{
		clear_lcd();
		DrawTitle_to_row(2, "no SIM card");
		WaitkeyAndAutoExit (100);		
