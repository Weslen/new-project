	...
	while (1)
	{
		...
		switch (keyval)
		{
			case KEY_UP:
				if (p != buff)
				{
					p = my_movePointToPrevLine (p, buff);
				}
	...
	while (1)
	{
		...
		switch (keyval)
		{
			...
			case KEY_UP:
				if (fromline >= 0)
				{
					pointUpALine (&pUp, buff, fromline);
					fromline--;
					flashflag = 1;
				}
...
void my_EnglishMonthToChinesMonth (char *Engmcm)
{
	...
	for (i = 0; i < 12; i++)
	{
		if (strcmp (Engmcm, EMCM[i][0]) == 0)
		{
			strcpy (Engmcm, EMCM[i][1]);
			break;
		}
	}
}
