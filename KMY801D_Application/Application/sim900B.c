...
static void PDU7bitEncode (char *buff)
{
	...
	while (*pbuff != 0)
	{
		...
		temp = *pbuff >> i | temp;
		*ptp = HexMapChar[temp >> 4];
		ptp++;
		*ptp = HexMapChar[temp & 0x0f];
		ptp++;
...
char SendMessage (char *Msg, const char *Phone)
{
	...
	if (ReadDatasFromSIM300_OKstr ( (unsigned char *) tempbuff, sizeof (tempbuff), 3, "OK\r") != 0)
	{
		goto tuichu;
	}
	...
	if (strstr (tempbuff, ">") )
	{
		...
		if (ReadDatasFromSIM300_OKstr ( (unsigned char *) tempbuff, sizeof (tempbuff) - 1024, 8, "OK\r") != 0)
		{
			goto tuichu;
		}
	}
...
char ReceiveMessageDecode (const char *codebuff, char *PhoneNumber, char *time, char *context, const char *type)
{
	...
	memcpy (time, "-", 1);
	time += 1;
	memcpy (time, temp + 2, 2);
	time += 2;
	memcpy (time, "-", 1);
	time += 1;
	memcpy (time, temp + 4, 2);
	time += 2;
	memcpy (time, " ", 1);
	time += 1;
	memcpy (time, temp + 6, 2);
	time += 2;
	memcpy (time, ":", 1);
	time += 1;
	memcpy (time, temp + 8, 2);
	time += 2;
	memcpy (time, ":", 1);
	time += 1;
	memcpy (time, temp + 10, 2);
	time += 2;
	*time = 0;
